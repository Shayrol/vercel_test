...
- **서버 및 클라이언트 캐싱**: 한국관광공사 API 응답 데이터를 CDN + 클라이언트 캐싱 처리하여 반복 요청을 방지하고 성능을 개선
...
- **Lighthout LCP 속도 상승**: CDN 캐싱을 통해 LCP가 1.1초에서 0.4초로 개선이 이루어젔으며 서버, 클라이언트 캐싱 사용으로 API 요청 횟수를 많이 줄였다.
